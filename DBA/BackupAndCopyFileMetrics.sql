/*
	...
	This script will get the total duration it took to take the BACKUP & COPY THE FILE OVER TO THE SHARED folders.
...
WHERE (backup_step LIKE 'STEP0%' OR backup_step LIKE 'END - BACKUP Process%') 
...
SELECT  SERVERNAME,DBNAME,BKPTYPE,SUM(RunTimeInSec)/COUNT(TotalRecords) AS AvgDurationInSec,
					 CONVERT(NUMERIC(10, 2), (SUM(RunTimeInSec)/60.0)/COUNT(TotalRecords)) AS AvgDurationInMins,
...
FROM #BACKUPTYPE
GROUP BY SERVERNAME,DBNAME,BKPTYPE,TotalRecords
ORDER BY 2
