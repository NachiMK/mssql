...
/*********   DB CONTEXT STATEMENT    *********/ 
  ...
  WHERE  [type] IN ( ''U'', ''S'', ''G'' ) -- windows users, sql users, windows groups 
         AND ((NAME = @UserToScript) OR (@UserToScript IS NULL))
         AND NAME != ''guest''
  UNION 
  /*********************************************/ 
  /*********    MAP ORPHANED USERS     *********/ 
  ...
  WHERE  rm.[type] IN ( ''U'', ''S'', ''G'' ) 
         ...
         AND  ((rm.NAME = @UserToScript) OR (@UserToScript IS NULL)) 
  UNION 
  /*********************************************/ 
  /*********    DB ROLE PERMISSIONS    *********/ 
  /*********************************************/ 
  ...
  FROM   sys.database_permissions AS perm 
         ...
         INNER JOIN sys.database_principals AS usr 
                 ON perm.grantee_principal_id = usr.principal_id 
                    AND ((usr.NAME = @UserToScript) OR (@UserToScript IS NULL)) 
  ...
  WHERE  class = 3 --class 3 = schema 
         AND ((dbprin.NAME = @UserToScript) OR (@UserToScript IS NULL)) 
...
WHILE @@FETCH_STATUS = 0 
  BEGIN 
      SET @SqlPermissions += ISNULL(@Sql, '''') + CHAR(13)
