...
/****** Object:  UserDefinedFunction [dbo].[fn_CSVToTable]    Script Date: 1/19/2017 8:50:44 AM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  UserDefinedFunction [dbo].[ufn_time_days_hours_minutes_seconds]    Script Date: 1/19/2017 8:50:53 AM ******/
SET ANSI_NULLS ON
GO
...
create FUNCTION [dbo].[ufn_time_days_hours_minutes_seconds] (@startdate datetime,@enddate datetime)
RETURNS TABLE
AS

/*
Use Case
declare @outdays varchar(max), @outhours varchar(max),@outminutes varchar(max),@outseconds varchar(max), @outms varchar(max)
SELECT @outdays=days, @outhours=hours,@outminutes=minutes,@outseconds=seconds,@outms=ms FROM [mnDBA].[dbo].[ufn_time_days_hours_minutes_seconds] (
   '2012-03-07 08:23:19.407'
  ,GETDATE())

print 'Process took ' + @outdays + ' days, ' + @outhours + ' hours, ' + @outminutes + ' minutes, ' + @outseconds + ' seconds, ' + @outms + ' ms.'   
*/
RETURN 
(
    select
...
/****** Object:  UserDefinedFunction [dbo].[ufn_time_days_hours_minutes_seconds_left]    Script Date: 1/19/2017 8:51:01 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[ufn_time_days_hours_minutes_seconds_left] (@oldnumber INT,@oldtime DATETIME,@newnumber INT,@newtime DATETIME=NULL,@remaining INT=null )
RETURNS @retTable TABLE
(
	outdays varchar(max)
	,outhours varchar(max)
	,outminutes varchar(max)
	,EstimatedCompletionTime varchar(max)
	,processed varchar(max)
	,outstring varchar(max)
	,rateperminute  varchar(max)
	,intervalinminutes varchar(max)
	,outseconds varchar(max)
...
/*
Use Case
  SELECT * FROM [mnDBA].[dbo].[ufn_time_days_hours_minutes_seconds_left] ( 
47201 
,'2013-12-12 11:38:21.537' 
,(SELECT COUNT(1) FROM dbo.logonmembercommunity_communityid20_deleted with (nolock)
...
declare @interval float,@processed int
declare @outdays float, @outhours float,@outminutes float,@outseconds float, @outms float,@rateperminute  float
,@EstimatedCompletionTime VARCHAR(MAX)

select @interval=ABS(datediff("MINUTE",@oldtime,@newtime))
select @processed=ABS(@oldnumber-@newnumber)
SELECT @rateperminute=@processed/@interval

SELECT @outminutes= @remaining / @rateperminute
SELECT @outhours =@outminutes/60  
select @outdays=@outhours/24.0  





SELECT @EstimatedCompletionTime=CONVERT(VARCHAR(26), DATEADD("minute",@outminutes,getdate()),109) 


declare @outdays1  VARCHAR(MAX), @outhours1  VARCHAR(MAX),@outminutes1  VARCHAR(MAX),@outstring VARCHAR(MAX)
SELECT @outdays1=days, @outhours1=hours,@outminutes1=minutes,@outseconds=seconds,@outms=ms FROM [mnDBA].[dbo].[ufn_time_days_hours_minutes_seconds] (
  @oldtime
  ,@newtime)



SELECT @outstring=@outdays1 + ' days, ' + @outhours1 + ' hours, ' + @outminutes1 + ' minutes, ' + CONVERT(VARCHAR(max),@outseconds) + ' seconds, ' + CONVERT(VARCHAR(max),@outms )+ ' ms.'   
...
/****** Object:  UserDefinedFunction [dbo].[fn_getlocaltime_fromUTC]    Script Date: 1/19/2017 8:51:13 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[fn_getlocaltime_fromUTC]
(
	-- Add the parameters for the function here
	@DateYouWantToConvert datetime
	--needed for sql 2005
	--,@utcdate datetime,@LocalDate datetime
)
RETURNS datetime
AS
BEGIN
...
/****** Object:  UserDefinedFunction [dbo].[udf_RemoveChars]    Script Date: 1/19/2017 8:51:25 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[udf_RemoveChars](@Input varchar(max))
RETURNS VARCHAR(max)
BEGIN
  DECLARE @pos INT
...
/****** Object:  UserDefinedFunction [dbo].[fn_greatest_date]    Script Date: 1/19/2017 8:51:36 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[fn_greatest_date] (
    @date_1 datetime
,   @date_2 datetime
,   @date_3 datetime
,   @date_4 datetime
...
BEGIN
    ...
    SELECT
        @date_1 = ISNULL(@date_1, '1900-01-01')
    ,   @date_2 = ISNULL(@date_2, '1900-01-01')
    ,   @date_3 = ISNULL(@date_3, '1900-01-01')
    ,   @date_4 = ISNULL(@date_4, '1900-01-01')
    ,   @date_5 = ISNULL(@date_5, '1900-01-01')
    ,   @date_6 = ISNULL(@date_6, '1900-01-01')

    SELECT  top 1 @return_date=dateval
    FROM
    (   SELECT @date_1 AS dateval UNION ALL
        SELECT @date_2 UNION ALL
        SELECT @date_3 UNION ALL
        SELECT @date_4 UNION ALL 
        SELECT @date_5 UNION ALL 
        SELECT @date_6 
    ) dates order by dateval desc
...
/****** Object:  UserDefinedFunction [dbo].[udf_RunDurationIntoSeconds]    Script Date: 1/19/2017 8:47:47 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[udf_RunDurationIntoSeconds]
(
      @run_duration INT
)
RETURNS INT
...
/****** Object:  StoredProcedure [dbo].[sp_WhoIsActive]    Script Date: 1/19/2017 8:45:29 AM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[usp_AuditTableSizes]    Script Date: 1/19/2017 8:45:12 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_AuditTableSizes] AS

SET NOCOUNT ON 
DECLARE @DBname NVARCHAR(MAX) 
DECLARE @InsertDate NVARCHAR(MAX) = CONVERT(NVARCHAR(MAX ),GETDATE(),121)
DECLARE @sql NVARCHAR(MAX)



DECLARE TableSizeAuditCursor CURSOR FORWARD_ONLY
...
OPEN TableSizeAuditCursor

FETCH NEXT FROM TableSizeAuditCursor INTO @DBname
...
EXECUTE sp_MSforeachtable ''insert into mnDBA.dbo.TableSizeAudit (tablename,ROWS,RESERVED,DATA,INDEXSIZE,UNUSED) EXECUTE sp_spaceused [?]''

update mnDBA.dbo.TableSizeAudit
...
--PRINT @DBname + ' inserted'

FETCH NEXT FROM TableSizeAuditCursor INTO @DBname

END

CLOSE TableSizeAuditCursor
DEALLOCATE TableSizeAuditCursor
...
/****** Object:  StoredProcedure [dbo].[usp_Check_Long_Running_Jobs]    Script Date: 1/19/2017 8:44:49 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_Check_Long_Running_Jobs] (
@p_threshold_minutes INT = 10
)


--EXEC [usp_Check_Long_Running_Jobs] 10

AS
SET NOCOUNT ON

declare @rowcount INT, @xml NVARCHAR(MAX), @body NVARCHAR(MAX),@subject nvarchar(2000),@alert_date_time DATETIME, @session_id INT
,@JMD_ID INT, @JOB_ID UNIQUEIDENTIFIER

SELECT @alert_date_time = GETDATE()
SELECT @session_id = MAX(session_id) FROM msdb.dbo.syssessions

--truncate table mnDBA.dbo.JobMonitorDetails
INSERT INTO mnDBA.dbo.JobMonitorDetails (session_id,job_id,start_execution_date,stop_execution_date,alert_date_time)
SELECT a.session_id,a.job_id,a.start_execution_date,a.stop_execution_date,@alert_date_time
FROM msdb.dbo.sysjobactivity a with (nolock)
JOIN msdb.dbo.sysjobs b with (nolock)
ON a.job_id = b.job_id
WHERE DATEDIFF(minute,a.start_execution_date,GETDATE()) > @p_threshold_minutes AND name LIKE 'Mingle Replicate%'
AND a.session_id = @session_id 

SELECT @rowcount = @@rowcount
--SELECT @JMD_ID = MAX(jmd_id) FROM mnDBA.dbo.JobMonitorDetails WITH (NOLOCK)

--SELECT @JOB_ID = job_id FROM mnDBA.dbo.JobMonitorDetails WITH (NOLOCK) WHERE jmd_id = @JMD_ID


IF @rowcount > 0
BEGIN
	--INSERT INTO mnDBA.dbo.ProcessShowTableStatusMYSQL
	--SELECT GETDATE(),* from
	--OPENQUERY(provo,'show open tables') a
	--WHERE a.[DATABASE] = 'christian_site'
	--AND a.[table] IN ('user','billing_recovery','user_migration','billing_package_item','services','billing_packages','campaign_track','billing_pending','billing_complete','extra','billing_note','billing_item','user_notes')

	--stop the job
	DECLARE @name VARCHAR(MAX)

	DECLARE cur_long_jobs CURSOR FAST_FORWARD READ_ONLY FOR
		SELECT b.name
		FROM mnDBA.dbo.JobMonitorDetails a with (nolock)
		JOIN msdb.dbo.sysjobs b with (nolock)
		ON a.job_id = b.job_id
		WHERE a.alert_date_time = @alert_date_time

	OPEN cur_long_jobs

	FETCH NEXT FROM cur_long_jobs INTO @name

	WHILE @@FETCH_STATUS = 0
	BEGIN
		
		IF EXISTS (SELECT 1 FROM     msdb..sysjobactivity ja ( NOLOCK )
        JOIN msdb..sysjobs j ( NOLOCK ) ON j.[job_id] = ja.[job_id]
		WHERE   ja.stop_execution_date IS NULL AND j.name = @name) 
	...
	CLOSE cur_long_jobs
	DEALLOCATE cur_long_jobs
	
	SET @xml =CAST(( 
	SELECT a.name AS 'td','',b.start_execution_date AS 'td','',@p_threshold_minutes AS 'td'
	FROM msdb.dbo.sysjobs a
	JOIN mnDBA.dbo.JobMonitorDetails b
	ON a.job_id = b.job_id
	WHERE b.alert_date_time = @alert_date_time
	FOR XML PATH('tr'), ELEMENTS ) AS NVARCHAR(MAX))

	
	IF @xml IS NOT NULL
	BEGIN
		SELECT @subject='SQL Agent Jobs Exceeding Run Time Threshold'
		
		SET @body ='<html><H1>Long Running Jobs Exceeding Threshold</H1><body bgcolor=grey><table border = 2><tr><th>job_name</th><th>start_execution_date</th><th>duration_threshold_minutes</th></tr>' 
		...
		@recipients =N'db@spark.net',@body = @body,@body_format ='HTML',@subject =@subject,@profile_name ='default'
...
/****** Object:  StoredProcedure [dbo].[usp_DatabaseGrowth_ViewLastN]    Script Date: 1/19/2017 8:44:36 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_DatabaseGrowth_ViewLastN] ( @days INT = 10 )
AS 
      SET NOCOUNT ON
      ...
      DECLARE P_Cursor CURSOR FORWARD_ONLY
              FOR SELECT DISTINCT
                  ...
                  FROM
                    [DBGrowthRate] AS dgr 
      OPEN P_Cursor
      FETCH NEXT FROM P_Cursor INTO @DBName
      WHILE @@FETCH_STATUS = 0
            BEGIN
                  INSERT INTO
                    @table
                    ...
                    WHERE
                        [DBName] = @DBName
                        AND [MetricDate] > DATEADD("d" , @days*-1 , GETDATE())
                    ORDER BY
                        [MetricDate] DESC  
                  FETCH NEXT FROM P_Cursor INTO @DBName
            END
	...
	SELECT dbname,MIN(origsize) MinOrigSize,MAX(CurSize) MaxSize,MAX(CurSize) -MIN(origsize) GrowthAmt
	FROM @table
	GROUP BY dbname
ORDER BY MAX(CurSize) -MIN(origsize)DESC 

/*
      SELECT
        [DBName],
        cast([OrigSize] / 1024 AS DECIMAL(10 , 1)) "ORIG_SIZE_GB",
        NULL AS [GrowthAmt],
        datename(dw, metricdate) + ' ' +
        DATENAME(MM, [MetricDate]) + RIGHT(CONVERT(VARCHAR(12), [MetricDate], 107), 9) AS [LastDay]
	INTO #lastday
      FROM
        @table
      WHERE
        [MetricDate] IN ( SELECT
                            MAX(metricdate)
                          FROM
                            @table
                          GROUP BY
	 ...
	 SELECT
        [DBName],
        cast([OrigSize] / 1024 AS DECIMAL(10 , 1)) "ORIG_SIZE_GB",
        (SELECT ORIG_SIZE_GB FROM #lastday l WHERE l.DBName=t.dbname) AS FinalSize,
        (SELECT ORIG_SIZE_GB FROM #lastday l WHERE l.DBName=t.dbname) -  cast([OrigSize] / 1024 AS DECIMAL(10 , 1)) AS [GrowthAmt],
        datename(dw, metricdate) + ' ' +
        DATENAME(MM, [MetricDate]) + RIGHT(CONVERT(VARCHAR(12), [MetricDate], 107), 9) AS [FirstDay]
	 ...
	 */
      ...
      SELECT
        [DBName],
        CAST([OrigSize] / 1024 AS DECIMAL(10 , 1)) "ORIG SIZE (GB)",
        CAST([CurSize] / 1024 AS DECIMAL(10 , 1)) "CURR SIZE (GB)",
        [GrowthAmt],
        DATENAME(dw, metricdate) + ' ' +
        DATENAME(MM, [MetricDate]) + RIGHT(CONVERT(VARCHAR(12), [MetricDate], 107), 9) AS [Month DD, YYYY],
		DataSize,
		LogSize
      FROM
        @table
...
/****** Object:  StoredProcedure [dbo].[usp_DatabaseGrowth_Update_Sizes]    Script Date: 1/19/2017 8:44:32 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_DatabaseGrowth_Update_Sizes] AS
SELECT sd.name AS DBName, mf.database_id, SUM(size) AS size, mf.type_desc
INTO #TempDBSize2
FROM sys.databases sd
JOIN sys.master_files mf
ON sd.database_ID = mf.database_ID
GROUP BY sd.name, mf.database_id,  mf.type_desc
ORDER BY mf.database_id, sd.name



IF EXISTS (SELECT DISTINCT a.DBName FROM #TempDBSize2 a JOIN DBGrowthRate b ON a.database_id = b.DBID AND A.DBName = B.DBName)
  BEGIN
		INSERT INTO dbo.DBGrowthRate (DBName, DBID, NumPages, OrigSize, CurSize, GrowthAmt, MetricDate,DataSize, LogSize)
		(SELECT tds.DBName, tds.database_ID, (tds.size + tdss.size)  AS NumPages, 
		dgr.CurSize AS OrigSize,
		CONVERT(DECIMAL(18,2),(((CONVERT(DECIMAL(18,2),(tds.size + tdss.size)) * 8000)/1024)/1024)) AS CurSize,
		CONVERT(VARCHAR(100),(CONVERT(DECIMAL(18,2),(((CONVERT(DECIMAL(18,2),(tds.size + tdss.size)) * 8000)/1024)/1024)) 
					- dgr.CurSize)) + ' MB' AS GrowthAmt, GETDATE() AS MetricDate
					,tds.size AS datasize, tdss.size AS logsize
		FROM #TempDBSize2 tds
		JOIN #TempDBSize2 tdss
		ON tds.database_id = tdss.database_id
		JOIN DBGrowthRate dgr
		ON tds.database_ID = dgr.DBID
		WHERE DBGrowthID = (SELECT DISTINCT MAX(DBGrowthID) FROM DBGrowthRate
														WHERE DBID = dgr.DBID)
															AND tds.type_desc = 'rows' AND tdss.type_desc = 'log'
		)
  END
IF EXISTS (SELECT DISTINCT a.DBName FROM #TempDBSize2 a LEFT JOIN DBGrowthRate b ON a.database_id = b.DBID  AND A.DBName = B.DBName WHERE b.DBID IS NULL)
		BEGIN
			INSERT INTO dbo.DBGrowthRate (DBName, DBID, NumPages, OrigSize, CurSize, GrowthAmt, MetricDate,DataSize, LogSize)
			(SELECT tds.DBName, tds.database_ID, (tds.size + tdss.size) AS NumPages, 
			CONVERT(DECIMAL(18,2),(((CONVERT(DECIMAL(18,2),(tds.size + tdss.size)) * 8000)/1024)/1024)) AS OrigSize,
			CONVERT(DECIMAL(18,2),(((CONVERT(DECIMAL(18,2),(tds.size + tdss.size)) * 8000)/1024)/1024)) AS CurSize,
...
/****** Object:  StoredProcedure [dbo].[usp_LongRunningJobCheck]    Script Date: 1/19/2017 8:44:04 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_LongRunningJobCheck] AS
...
IF OBJECT_ID('tempdb.dbo.#HistoryTable') IS NOT NULL 
    DROP TABLE #HistoryTable


SELECT DISTINCT
        ja.job_id JobId ,
        DATEDIFF(second, ja.start_execution_date, GETDATE()) RunningSeconds ,
        j.name JobName
INTO    #HistoryTable
FROM    msdb..sysjobactivity ja ( NOLOCK )
        JOIN msdb..sysjobs j ( NOLOCK ) ON j.[job_id] = ja.[job_id]
WHERE   ja.stop_execution_date IS NULL
        AND ja.start_execution_date > DATEADD(DAY, -10, GETDATE())
			and j.category_id NOT IN (10, 13)															-- Exclude REPL-Distribution and REPL-LogReader categories, these are continuous running jobs.
				AND ja.session_id IN (SELECT MAX(session_id) FROM msdb..sysjobactivity WITH (NOLOCK))	-- Only looks at the current session ID
					AND j.enabled = 1																	-- Only check for enabled jobs
					


IF OBJECT_ID('tempdb.dbo.#AverageTable') IS NOT NULL 
    DROP TABLE #AverageTable

SELECT  AVG(mnDBA.dbo.udf_RunDurationIntoSeconds(sh.run_duration)) AverageSeconds ,
        ht.JobId ,
        ht.JobName ,
        MAX(ht.RunningSeconds) RunningSeconds
INTO    #AverageTable
FROM    #HistoryTable ht
        JOIN msdb..sysjobhistory sh ON [sh].[job_id] = ht.JobId
WHERE   step_name = '(job outcome)'
			AND sh.run_status = 1      -- Only look at history of successfully completed jobs.
GROUP BY ht.JobId ,
        ht.JobName 


IF OBJECT_ID('tempdb.dbo.#AlertsTable') IS NOT NULL 
    DROP TABLE #AlertsTable
...
INSERT  INTO #AlertsTable
        SELECT  *
        FROM    #AverageTable
        WHERE   AverageSeconds BETWEEN 1 AND 10
                AND AverageSeconds * 30 < RunningSeconds


INSERT  INTO #AlertsTable
        SELECT  *
        FROM    #AverageTable
        WHERE   AverageSeconds > 10
                AND AverageSeconds <= 60
                AND AverageSeconds * 6 < RunningSeconds


INSERT  INTO #AlertsTable
        SELECT  *
        FROM    #AverageTable
        WHERE   AverageSeconds > 60
                AND AverageSeconds <= 300
                AND AverageSeconds * 4 < RunningSeconds


INSERT  INTO #AlertsTable
        SELECT  *
        FROM    #AverageTable
        WHERE   AverageSeconds > 300
                AND AverageSeconds <= 1500
                AND AverageSeconds * 3 < RunningSeconds


INSERT  INTO #AlertsTable
        SELECT  *
        FROM    #AverageTable
        WHERE   AverageSeconds > 1500
                AND AverageSeconds * 2 < RunningSeconds
...
DECLARE @messagetitle NVARCHAR(MAX)
DECLARE @messagebody NVARCHAR(MAX)


DECLARE LongRunningJob CURSOR FORWARD_ONLY
FOR

SELECT  JobName, AverageSeconds, RunningSeconds
FROM    #AlertsTable
WHERE   AverageSeconds < RunningSeconds


OPEN longrunningjob
FETCH NEXT FROM LongRunningJob 
INTO @Name, @averagetime, @executiontime

WHILE @@FETCH_STATUS = 0 
    BEGIN

        SET @messagetitle = 'SQL Agent Long Running Job: [' + @name + ']'
        SET @messagebody = '[' + @name + '] is running longer than average.'
            + CHAR(13) + CHAR(13) + 'Current Execution Time: '
            + CAST(@executiontime AS VARCHAR) + ' Seconds.' + CHAR(13)
            + 'Average Execution Time: ' + CAST(@averagetime AS VARCHAR)
            + ' Seconds.'
        EXEC msdb.dbo.sp_send_dbmail @profile_name = 'admin',
            @recipients = 'db@spark.net', @subject = @messagetitle,
            @body = @messagebody
...
CLOSE LongRunningJob
DEALLOCATE LongRunningJob
GO
...
/****** Object:  StoredProcedure [dbo].[usp_PurgeJobHistory_Cursor]    Script Date: 1/19/2017 8:43:54 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_PurgeJobHistory_Cursor]
...
BEGIN
	...
	BEGIN
		...
		--> no such job?
		EXECUTE @retval = msdb.dbo.sp_verify_job_identifiers '@job_name', '@job_id', @job_name OUTPUT, @job_id OUTPUT, @owner_sid = @job_owner_sid OUTPUT
		IF (@retval <> 0)
			RETURN(1) -- Failure
		...
		/*	Eyal Z. 2012-06-28
		 	Modified original non-batched delete oporation as cursor.
		-------------------------------------------------------------
		DELETE	FROM msdb.dbo.sysjobhistory
		WHERE	(job_id = @job_id) 
		AND		((run_date < @datepart) 
					OR (run_date <= @datepart AND run_time < @timepart)
				)
		...
		*/ 
		...
		/* Changed to row-by-row deletes */ 
		DECLARE c CURSOR FAST_FORWARD 
		FOR 
			SELECT instance_id FROM msdb.dbo.sysjobhistory (nolock) 
			WHERE (job_id = @job_id) AND
			((run_date < @datepart) OR 
			(run_date <= @datepart AND run_time < @timepart))
				AND instance_id NOT IN (SELECT TOP (@history_lines) instance_id FROM msdb.dbo.sysjobhistory (NOLOCK) -- new block of code designed to keep at least x amount of history lines past 20 hours
										WHERE job_id = @job_id
										ORDER BY instance_id DESC)

		OPEN c 

		FETCH NEXT FROM c INTO @instance_id 
		WHILE @@FETCH_STATUS = 0
		BEGIN 
			-- DELETE ROW BY ROW 
			DELETE FROM msdb.dbo.sysjobhistory WITH (ROWLOCK)  
	...
	-- Only a sysadmin or SQLAgentOperatorRole can do this
		...
		BEGIN
			...
			*/ 
			...
			FOR 
				...
				--(run_date <= @datepart AND run_time < @timepart))
				--	AND instance_id NOT IN (SELECT TOP (@executions) instance_id FROM msdb.dbo.sysjobhistory (NOLOCK)
				--						WHERE step_name = '(Job outcome)'
				--							--	AND job_id = @job_id
				--						ORDER BY instance_id DESC)
				...
				--with top10_jobs as ( 
				-- select instance_id,job_id, step_name,  row_number() over (partition by job_id order by instance_id desc) as row_num  
				-- from jobhistory with (nolock)
				-- WHERE step_name = '(Job outcome)') 


				SELECT instance_id FROM msdb.dbo.sysjobhistory j1 (NOLOCK)
				WHERE ((run_date < @datepart) OR 
				(run_date <= @datepart AND run_time < @timepart))
					AND instance_id not IN (select TOP (@history_lines) instance_id -- new block of code designed to keep at least x amount of history lines past 20 hours
		...
		BEGIN
			...
			*/ 
			...
			-- save count before truncate. 
			SELECT @rows_affected = COUNT(1) FROM msdb.dbo.sysjobhistory (NOLOCK) 
			TRUNCATE TABLE msdb.dbo.sysjobhistory  

		END

		/* Removed due to cursor based deletes 
		SELECT @rows_affected = @@rowcount
		*/ 
...
/****** Object:  StoredProcedure [dbo].[usp_PurgeJobHistory]    Script Date: 1/19/2017 8:43:23 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[usp_PurgeJobHistory]
	@hours INT = 20 
AS
DECLARE @dt DATETIME
SET @dt=DATEADD("hour",-@hours,GETDATE())
EXEC mnDBA.dbo.usp_PurgeJobHistory_Cursor  @oldest_date=@dt
...
/****** Object:  StoredProcedure [dbo].[usp_DisableOrEnableJobs]    Script Date: 1/19/2017 8:43:11 AM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[usp_EnsureAllDatabaseHaveTriggers]    Script Date: 1/19/2017 8:44:20 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_EnsureAllDatabaseHaveTriggers] AS  
set nocount on  
DECLARE @name VARCHAR(max)
...
DECLARE P_Cursor CURSOR FORWARD_ONLY
        FOR SELECT
			...
			WHERE state = 0
OPEN P_Cursor
FETCH NEXT FROM P_Cursor INTO @name
WHILE @@FETCH_STATUS = 0
      BEGIN
