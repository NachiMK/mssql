...
OUTER APPLY	(
			...
			AND			(
							(RTC.name	= LTC.name)
						OR	((REPLACE(LTC.name, '_FK', '_BK') = RTC.name) AND LTT.name = 'int' AND RTC.is_identity = 0)
						OR	((REPLACE(LTC.name, '_FK', '_SK') = RTC.name) AND LTT.name = 'int' AND RTC.is_identity = 0)
						)
			) AS RTD
WHERE		RTS.name	= 'DM'
AND			RT.name		= 'DistrictEmployee'
