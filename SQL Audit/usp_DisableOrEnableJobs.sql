...
/*
	This script creates a table in DBATools and stores the jobs that are being disabled or enabled. Make sure you change the Reason, Action and Debug flag to properly run.
...
*/
	...
	IF @Action = 'DISABLE'
	BEGIN
		-- DROP TABLE DBATools.dbo.DisableOrEnableJobLog
		IF OBJECT_ID('DBATools.dbo.DisableOrEnableJobLog') IS NULL
		BEGIN
			CREATE TABLE DBATools.dbo.DisableOrEnableJobLog
			(
			...
			CREATE UNIQUE NONCLUSTERED INDEX [UNQ_DisableEnableJob] ON DBATools.dbo.DisableOrEnableJobLog(Reason, ServerName, job_id)
		END
		...
		BEGIN
			...
			INSERT INTO DBATools.dbo.DisableOrEnableJobLog
		...
		IF @Debug = 1
			SELECT 'DISABLED' AS Comments, * FROM DBATools.dbo.DisableOrEnableJobLog WHERE Reason = @BatchName AND ServerName = @@SERVERNAME
	...
	BEGIN
		...
		BEGIN
			...
			INTO	#EnableJobs
			FROM	DBATools.dbo.DisableOrEnableJobLog
			...
			BEGIN
				...
				UPDATE	DBATools.dbo.DisableOrEnableJobLog
			...
			IF @Debug = 1
				SELECT 'Enabled' AS Comments, * FROM DBATools.dbo.DisableOrEnableJobLog WHERE Reason = @BatchName AND ServerName = @@SERVERNAME
		END
	END
	ELSE
	BEGIN
	...
	IF @Debug = 1
		...
		FROM	msdb.dbo.sysjobs SJ
		WHERE	EXISTS (SELECT 1 FROM DBATools.dbo.DisableOrEnableJobLog WHERE Reason = @BatchName AND ServerName = @@SERVERNAME AND SJ.job_id = job_id)
